...
knitr::opts_chunk$set(
  dev = c("png"), # Use "cairo_pdf" for high-quality plots with searchable text (works for Windows but may not work for Mac users). If it works, change to "cairo_pdf".
  dpi = 600,
  fig.align = "center",
  out.width = "100%",
  comment = NA,
...
(p = `r round(plant_aov_summary[[1]][["Pr(>F)"]][[1]], 3)`). Plants with no chemical exposure weighed `r plant_weights %>% filter(group == "ctrl") %>% pull(mean_weight) %>% signif(.,3)` g.
